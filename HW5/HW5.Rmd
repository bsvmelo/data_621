...
```{r, echo=FALSE, warning=FALSE}
Model5 <- glm.nb(TARGET ~ ., data = complete_train_data)
summary(Model5)
```
...
To aid in model selection, let's test each of our models agains the complete_train_data set. 

```{r, echo=FALSE, warning=FALSE}

getMAE <- function(x) {
    mean(abs(complete_train_data$TARGET - x))
}
getRMSE <- function(x) {
    sqrt(mean((complete_train_data$TARGET - x)^2))
}
results <- data.frame(Model = c("Poisson1",
                                ...
                                "Negative-binomial-regression",
                                "Zero-Inflated-Count-Models"), 
                     MAE = c(getMAE(predict.lm(Model1, complete_train_data)),
                             getMAE(predict.lm(Model2, complete_train_data)),
                             getMAE(predict.lm(Model3, complete_train_data)),
                             getMAE(predict.lm(Model4, complete_train_data)),
                             getMAE(predict(Model5, complete_train_data,
                                            type="response")),
                             getMAE(predict(Model6, complete_train_data,
                                            type="response"))
                             ),
                     RMSE = c(getRMSE(predict.lm(Model1, complete_train_data)),
                              getRMSE(predict.lm(Model2, complete_train_data)),
                              getRMSE(predict.lm(Model3, complete_train_data)),
                              getRMSE(predict.lm(Model4, complete_train_data)),
                              getRMSE(predict(Model5, complete_train_data,
                                            type="response")),
                              getRMSE(predict(Model6, complete_train_data,
                                            type="response"))
                              )
)
knitr::kable(results)
...
In the table above, we see a preference for our Zero Inflated Count Model. We'll make our predictions using our the Zero Inflated Count model which is Model6
...
```{r, echo=FALSE, warning=FALSE}
finalpreds <- predict(Model6, complete_eval_data)
