...
Looking at the plots we see we have to make a few changes to some variables. We'll make HOMEKIDS boolean instead of a factor. For the rows where CAR_AGE are labeled numbers, we make 0. For blank JOBS we label those as "Unknown". Finally, change Education to 1 if PhD and Masters.
...
Eval_Data$TARGET_AMT = NULL

Eval_Data$INDEX = NULL
prediction_binary = predict(m4b, Eval_Data, type="response")
Eval_Data$TARGET_FLAG = prediction_binary
Eval_Data$TARGET_FLAG <- ifelse(Eval_Data$TARGET_FLAG > 0.5, 1, 0)
```

Multi Linear Model:
```{r}
prediction_linear = predict(mlr3, Eval_Data)
Eval_Data$TARGET_AMT = prediction_linear
